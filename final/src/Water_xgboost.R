...
test<-read.csv("C:/Users/Ray/Desktop/final/R/Pump-it-Up-XGBoost-Ensemble-master/test.csv")

#Create new column in test 
test$status_group <- 0

#Read in train and label datasets
train<-read.csv("C:/Users/Ray/Desktop/final/R/Pump-it-Up-XGBoost-Ensemble-master/train.csv")
label<-read.csv("C:/Users/Ray/Desktop/final/R/Pump-it-Up-XGBoost-Ensemble-master/TrainingLabels.csv")
...
write.csv(data_test.noID, file = "C:/Users/Ray/Desktop/final/R/Pump-it-Up-XGBoost-Ensemble-master/test_matrix.csv", row.names = FALSE)
data_train <- as.matrix(as.data.frame(lapply(data_train, as.numeric)))
write.csv(data_train, file = "C:/Users/Ray/Desktop/final/R/Pump-it-Up-XGBoost-Ensemble-master/train_matrix.csv", row.names = FALSE)
label<-as.numeric(label$status_group)
write.csv(label, file = "C:/Users/Ray/Desktop/final/R/Pump-it-Up-XGBoost-Ensemble-master/train_label_matrix.csv", row.names = FALSE)
...
#train.DMatrix.save_binary("train_matrix")
...
xgb.tab = xgb.cv(data = train.DMatrix, objective = "multi:softmax", booster = "gbtree",
                 ...
                 evaluation = "merror", eta = .2, max_depth = 14, colsample_bytree = .4)

#Create variable that identifies the optimal number of iterations for the model
#print(xgb.tab["best_ntreelimit"])
#min.error.idx <- int(xgb.tab["best_ntreelimit"])
#print()
#Create model using the same parameters used in 
xgb.cv
model <- xgboost(data = train.DMatrix, objective = "multi:softmax", booster = "gbtree",
                 eval_metric = "merror", nrounds =70, 
                 num_class = 4,eta = .2, max_depth = 14, colsample_bytree = .4)
#model.save_model("001.model")
outstr<-paste("best",i,".model",sep = "")
xgb.save(model, outstr)
...
head(solution)
xgb.save(model, 'best.model')
#Create csv submission file
write.csv(solution, file = "C:/Users/Ray/Desktop/final/R/Pump-it-Up-XGBoost-Ensemble-master/result4.csv", row.names = FALSE)
