...
def main():
    ...
    for i in range(train_tag.shape[0]):
        for j in range(train_tag.shape[1]):
            if train_tag[i][j] == 1.0:
                count[j] = count[j]+1
    print ('count = ',count)
    ...
    model.add(Dense(300,input_dim=len(word_index)+1,activation='relu'))
   ...
   # model.add(Dropout(0.5))
    ...
    earlystopping = EarlyStopping(monitor='val_f1_score', patience = 3, verbose=1, mode='max')
    checkpoint = ModelCheckpoint(filepath='bow_best.hdf5',
                                 verbose=1,
                                 save_best_only=True,
    ...
    thresh = 0.4
    with open('bagofwords_answer.csv','w') as output:
